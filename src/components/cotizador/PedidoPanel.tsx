...
export function PedidoPanel({ texto, observaciones, loading, exportando, puedeExportar, onTextoChange, onObservacionesChange, onGenerar, onExportar, onExportarPdf }: Props) {
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5">
      <label className="font-bold">Pedido del cliente / WhatsApp</label>
      <textarea value={texto} onChange={(event) => onTextoChange(event.target.value)} className="mt-4 w-full h-[440px] rounded-xl bg-slate-950 border border-slate-600 p-4 font-mono text-sm" />
